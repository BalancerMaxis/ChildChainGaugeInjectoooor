    ...
    Initializable
{
    ...
    error PaymentAmountOverGlobalMax(
        address gaugeAddress,
    ...
    function addRecipients(
        ...
        if (amountPerPeriod > MaxInjectionAmount) {
            revert PaymentAmountOverGlobalMax(
                recipients[0],
                amountPerPeriod,
                MaxInjectionAmount
            );
        ...
        for (uint i = 0; i < recipients.length; i++) {
            ...
            if (
                gauge.reward_data(InjectTokenAddress).distributor !=
                address(this)
            ) {
    ...
    function getBalanceDelta() public view returns (int256 delta) {
        ...
        delta =
            int256(IERC20(InjectTokenAddress).balanceOf(address(this))) -
            int256(totalDue);
        // delta returned
    }
    ...
    function getReadyGauges() public view returns (address[] memory) {
        ...
        for (uint256 idx = 0; idx < gaugeList.length; idx++) {
            ...
            if (
                target.lastInjectionTimeStamp + minWaitPeriod <=
                block.timestamp &&
    ...
    function _injectFunds(address[] memory ready) internal whenNotPaused {
        ...
        for (uint256 idx = 0; idx < ready.length; idx++) {
            ...
            if (
                target.lastInjectionTimeStamp + minWaitPeriodSeconds <=
                block.timestamp &&
                period_finish <= block.timestamp &&
                balance >= target.amountPerPeriod &&
                target.periodNumber < target.maxPeriods &&
